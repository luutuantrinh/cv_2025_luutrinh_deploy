  ...
  /------------------------------------------------------*/
		...
		if ($("#contact-form").length > 0) {
			$("#contact-form").validate({
				...
				submitHandler: function (form) {
					...
					// });
					emailjs.sendForm(
						'service_zodut1l',     // Service ID
						'template_ngklsmd',    // Template ID
						'#contact-form',       // Form selector
						'XAbXOoIX16cXXbJ0b'    // Public key (User ID)
	...
	$(document).ready(function () {
		/*------------------------------------------------------
		  ...
		  /------------------------------------------------------*/
		// Function to open the next or previous portfolio popup
		function openPortfolioPopup(target) {
			if ($(target).length) {
				$.magnificPopup.open({
					items: {
						src: target
		...
		$('.navigation_item.next-project a').on('click', function (e) {
			e.preventDefault();
			var target = $(this).attr('href');
			$.magnificPopup.close(); // Close the current popup
			setTimeout(function() {
				openPortfolioPopup(target); // Open the next popup
			}, 300); // Delay to ensure the current popup is closed
		});
	
		// Event listener for previous project button
		$('.navigation_item.prev-project a').on('click', function (e) {
			e.preventDefault();
			var target = $(this).attr('href');
			$.magnificPopup.close(); // Close the current popup
			setTimeout(function() {
				openPortfolioPopup(target); // Open the previous popup
			}, 300); // Delay to ensure the current popup is closed
		});
	});
})(jQuery);
